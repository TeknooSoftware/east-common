...
namespace Teknoo\East\Website\Gedmo\Mapping;
...
use Teknoo\East\Website\Gedmo\Mapping\Event\AdapterInterface;
...
abstract class MappedEventSubscriber implements EventSubscriber
{
     ...
     *
     * @throws \Teknoo\East\Website\Gedmo\Exception\InvalidArgumentException - if event is not recognized
     *
     * @return \Teknoo\East\Website\Gedmo\Mapping\Event\AdapterInterface
     */
    protected function getEventAdapter(EventArgs $args)
    {
        $class = get_class($args);
        if (preg_match('@Doctrine\\\([^\\\]+)@', $class, $m) && in_array($m[1], array('ODM'))) {
            if (!isset($this->adapters[$m[1]])) {
                $adapterClass = $this->getNamespace().'\\Mapping\\Event\\Adapter\\'.$m[1];
                if (!class_exists($adapterClass)) {
                    $adapterClass = 'Teknoo\East\Website\Gedmo\\Mapping\\Event\\Adapter\\'.$m[1];
                }
            ...
            throw new \Teknoo\East\Website\Gedmo\Exception\InvalidArgumentException('Event mapper does not support event arg class: '.$class);
        }
    }
        ...
        if (null === self::$defaultAnnotationReader) {
            if (version_compare(\Doctrine\Common\Version::VERSION, '2.2.0-DEV', '>=')) {
                ...
                \Doctrine\Common\Annotations\AnnotationRegistry::registerAutoloadNamespace(
                    'Teknoo\East\Website\Gedmo\\Mapping\\Annotation',
                    __DIR__.'/../../'
                );
