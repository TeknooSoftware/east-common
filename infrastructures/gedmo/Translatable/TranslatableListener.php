...
use Teknoo\East\Website\Gedmo\Tool\Wrapper\AbstractWrapper;
use Teknoo\East\Website\Gedmo\Mapping\MappedEventSubscriber;
use Teknoo\East\Website\Gedmo\Translatable\Mapping\Event\TranslatableAdapter;
...
class TranslatableListener extends MappedEventSubscriber
{
     ...
     * @param object $om
     *
     * @throws \Teknoo\East\Website\Gedmo\Exception\RuntimeException - if language or locale property is not
     *                                           found in entity
     * @return string
     */
    public function getTranslatableLocale($object, $meta, $om = null)
    {
        ...
        if (isset(self::$configurations[$this->name][$meta->name]['locale'])) {
            ...
            if (!$reflectionProperty) {
                ...
                throw new \Teknoo\East\Website\Gedmo\Exception\RuntimeException("There is no locale or language property ({$column}) found on object: {$meta->name}");
            }
     ...
     *
     * @throws \Teknoo\East\Website\Gedmo\Exception\InvalidArgumentException if locale is not valid
     */
    protected function validateLocale($locale)
    {
        if (!$this->isValidLocale($locale)) {
            throw new \Teknoo\East\Website\Gedmo\Exception\InvalidArgumentException('Locale or language cannot be empty and must be set through Listener or Entity');
        }
    }
